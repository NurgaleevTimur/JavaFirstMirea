package lists_1.task2;

public class Group {
    private Student student;

    public Group() {
        this.student = new Student("", "", 0, null, null);
    }
    Student getFirst(){
        return student;
    }
    ...
    public void insert(String name, String group, int age){
        if (this.getLast().getNext()==null) {
            this.getLast().setNext(new Student(name, group, age, null, this.getLast()));
        }
        else{
            this.getLast().setNext(new Student(name, group, age, this.getLast().getNext(), this.getLast()));
        }
    }
